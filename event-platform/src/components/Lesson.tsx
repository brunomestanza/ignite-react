...
export function Lesson({ title, slug: linkSlug, availableAt, type }: LessonProps) {
  const { slug } = useParams<{ slug: string }>();
  const isLessonAvailable = isPast(availableAt);
  const availableDateFormatted = format(availableAt, "EEEE' • 'd' de 'MMMM' • 'k'h'mm",  { locale: ptBr });
  const isActiveLesson = slug === linkSlug;

  return (
    <Link to={`/event/lesson/${linkSlug}`} className='group'>
